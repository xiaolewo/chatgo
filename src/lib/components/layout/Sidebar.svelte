...
>
	...
	>
		<!-- 顶部标题区域 -->
		<div
			class="flex items-center justify-between px-2 py-1.5 border-b border-gray-200 dark:border-gray-700"
		>
		...
		<div class="p-2">
			<a
				...
				class="flex items-center justify-center gap-2 w-full py-1.5 px-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors font-medium shadow-sm"
				href="/"
				draggable="false"
				on:click={async () => {
					selectedChatId = null;
		...
		<!-- 搜索栏 -->
		<div class="px-2 py-1.5 {$temporaryChatEnabled ? 'opacity-20' : ''}">
		...
		<div class="mt-auto border-t border-gray-200 dark:border-gray-700">
			...
			{#if $user !== undefined && $user !== null}
				<div class="p-2 w-full flex flex-col pt-0">
					<UserMenu
						role={$user?.role}
						on:show={(e) => {
					...
					>
						<button
							class="flex w-full items-center gap-3 w-full p-3 text-left hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors"
							on:click={() => {
