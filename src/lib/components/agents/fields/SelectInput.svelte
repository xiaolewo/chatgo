	...
	<div class="relative">
		{#if searchable || multiple}
			...
			<div class="relative" class:readonly>
				...
				>
					<span class="flex-1 text-base text-gray-900 dark:text-white overflow-hidden text-ellipsis whitespace-nowrap">
					...
					<div class="flex items-center gap-2 flex-shrink-0">
						...
						{/if}
						<span class="text-gray-400 text-xs transition-transform duration-200 {isOpen ? 'rotate-180' : ''}">▼</span>
					</div>
				</div>

				{#if isOpen && !readonly}
					<div class="absolute top-full left-0 right-0 z-50 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-600 border-t-0 rounded-b-lg shadow-xl max-h-52 overflow-hidden">
						{#if searchable}
							...
							{#each filteredOptions as option}
								<div
									class="px-4 py-3 cursor-pointer flex items-center gap-2 text-base text-gray-900 dark:text-white transition-colors duration-200
										{isOptionSelected(option.value) ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-300 font-medium' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}
										{option.disabled ? 'text-gray-400 dark:text-gray-500 cursor-not-allowed' : ''}"
									on:click={() => !option.disabled && (multiple ? 
										handleMultipleChange(option.value, !isOptionSelected(option.value)) :
										(value = option.value, dispatch('change', option.value), isOpen = false)
									)}
								>
									{#if multiple}
										<input
