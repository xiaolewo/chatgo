<script>
	...
	import { getTemplateOptions, getTemplates, createPptTask, generateContent, generatePpt, getUserPpts, downloadPpt as downloadPptApi, GENERATION_TYPES } from '$lib/apis/ppt.js';
	...
	const scenes = [
		'通用场景', '教学课件', '工作总结', '工作计划', '项目汇报', 
		'解决方案', '研究报告', '会议材料', '产品介绍', '公司介绍', 
		'商业计划书', '科普宣传', '公众演讲'
	];
	...
	const getTaskTypeByGeneration = (genType) => {
		switch (genType) {
			case 'topic': return GENERATION_TYPES.TOPIC;
			case 'file': return GENERATION_TYPES.FILE;
			case 'markdown': return GENERATION_TYPES.MARKDOWN;
			case 'url': return GENERATION_TYPES.URL;
			default: return GENERATION_TYPES.TOPIC;
		}
	};
	...
	const generatePptTask = async () => {
		...
		try {
			...
			const markdown = contentResponse.data?.markdown || contentResponse.data?.content || contentResponse.data;
			...
			const pptId = pptResponse.data?.id || 
	...
	const getContentByType = () => {
		switch (generationType) {
			...
			case 'markdown': return markdownInput;
			case 'url': return urlInput;
			case 'file': return fileInput?.name || '';
			default: return '';
		}
	};
	...
	const downloadPptFile = async (task) => {
		try {
			...
			console.log('可用的下载ID:', { pptId: task.pptId, id: task.id, taskId: task.taskId, finalId: downloadId });
...
<div class="flex flex-col h-screen bg-white dark:bg-gray-900 overflow-hidden">
	<!-- 头部 -->
	<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between px-3 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-gray-700 space-y-2 sm:space-y-0">
		...
		<div class="flex flex-col sm:flex-row items-start sm:items-center space-y-1 sm:space-y-0 sm:space-x-3 text-xs sm:text-sm text-gray-600 dark:text-gray-400 w-full sm:w-auto">
	...
	<div class="flex flex-col lg:flex-row flex-1 overflow-auto lg:overflow-hidden min-h-0">
		...
		<div class="flex-1 flex flex-col min-h-0 overflow-auto lg:overflow-hidden">
					...
					<div>
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
							{#each generationTypes as type}
								<div
									class="relative cursor-pointer rounded-lg border p-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors {generationType === type.value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700'}"
			...
			<!-- 模板选择 -->
			<div class="flex-1 p-3 sm:p-6 overflow-y-auto min-h-0 lg:min-h-0" style="min-height: 400px;">
				<div class="max-w-4xl mx-auto">
					<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 space-y-3 sm:space-y-0">
						<h2 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">选择模板</h2>
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
						{#each templates as template}
							<div
								class="relative group cursor-pointer rounded-lg border overflow-hidden hover:shadow-lg transition-shadow {templateId === template.id ? 'border-blue-500 ring-2 ring-blue-200 dark:ring-blue-800' : 'border-gray-200 dark:border-gray-700'}"
							...
							>
								<div class="aspect-video bg-gray-200 dark:bg-gray-700 relative overflow-hidden">
									...
									<div 
										...
										style="display: {template.coverUrl && template.coverUrl.trim() ? 'none' : 'flex'};"
									>
			...
			<!-- 底部生成按钮 -->
			<div class="p-3 sm:p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex-shrink-0">
		...
		<!-- 右侧任务列表 -->
		<div class="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex flex-col max-h-80 lg:max-h-none">
			<div class="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
				<div class="flex items-center justify-between mb-3">
					<h2 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">生成历史</h2>
			...
			<div class="flex-1 overflow-y-auto">
				{#each taskList as task (task.id)}
					<div class="p-2 sm:p-3 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
						<div class="flex flex-col sm:flex-row sm:items-start justify-between space-y-2 sm:space-y-0">
							<div class="flex-1 min-w-0">
								<div class="font-medium text-xs sm:text-sm text-gray-900 dark:text-white truncate">
									{task.subject}
								</div>
								<div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
								...
								<div class="flex items-center mt-2 space-x-2">
									<div class="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-xs rounded-full">
