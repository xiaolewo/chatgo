        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                if result.get("code") == 0:
                    ...
                    return {"message": f"API错误: {result.get('message')}", "status": "error"}
            else:
...
async def generate_video(request: KlingGenerateRequest):
    ...
    # 如果配置了真实的API，尝试调用
    if CONFIG["KLING_API_URL"] != "your-api-url-here" and CONFIG["KLING_API_KEY"] != "your-api-key-here":
        try:
            # 构建API请求
            api_params = {
                ...
                if response.is_success:
                    ...
                    if api_result.get("code") == 0:
                        ...
                        return {
                            ...
                            "credits_used": CONFIG["KLING_PRO_CREDITS"] if request.mode == "pro" else CONFIG["KLING_STD_CREDITS"]
                        }
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.is_success:
                        ...
                        if api_result.get("code") == 0:
                            task_data = api_result["data"]
                            task["status"] = task_data.get("task_status", task["status"])
                            task["updated_at"] = task_data.get("updated_at", int(time.time() * 1000))
