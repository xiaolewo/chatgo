...
async def test_polling_process():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            submit_response = await client.post(submit_url, json=payload, headers=headers, timeout=30)
            print(f"提交状态码: {submit_response.status_code}")
            
            if submit_response.status_code == 200:
                submit_result = submit_response.json()
                print(f"提交结果: {json.dumps(submit_result, indent=2, ensure_ascii=False)}")
                ...
                if submit_result.get("code") == 1:
                    ...
                    # 步骤4: 模拟轮询过程
                    print(f"\n📋 步骤4: 模拟轮询过程（最多5次）")
                    await simulate_polling(client, API_URL, API_KEY, task_id, max_polls=5)
                    
                else:
                    print(f"❌ 任务提交失败: {submit_result}")
            else:
                print(f"❌ HTTP错误: {submit_response.status_code} - {submit_response.text}")
...
async def test_task_query(client, api_url, api_key, task_id):
    ...
    try:
        ...
        if query_response.status_code == 200:
            ...
            print(f"   查询结果: {json.dumps(query_result, indent=4, ensure_ascii=False)}")
