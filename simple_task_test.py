...
async def mock_process_midjourney_task(task_id: str):
    ...
    try:
        ...
        task_info["actions"] = [
            {"label": "U1", "custom_id": f"MJ::JOB::upsample::1::{task_id}", "type": "upscale", "emoji": "🔍"},
            {"label": "U2", "custom_id": f"MJ::JOB::upsample::2::{task_id}", "type": "upscale", "emoji": "🔍"},
            {"label": "U3", "custom_id": f"MJ::JOB::upsample::3::{task_id}", "type": "upscale", "emoji": "🔍"},
            {"label": "U4", "custom_id": f"MJ::JOB::upsample::4::{task_id}", "type": "upscale", "emoji": "🔍"},
            {"label": "V1", "custom_id": f"MJ::JOB::variation::1::{task_id}", "type": "variation", "emoji": "🎨"},
            {"label": "V2", "custom_id": f"MJ::JOB::variation::2::{task_id}", "type": "variation", "emoji": "🎨"},
            {"label": "V3", "custom_id": f"MJ::JOB::variation::3::{task_id}", "type": "variation", "emoji": "🎨"},
            {"label": "V4", "custom_id": f"MJ::JOB::variation::4::{task_id}", "type": "variation", "emoji": "🎨"},
            {"label": "🔄", "custom_id": f"MJ::JOB::reroll::0::{task_id}", "type": "reroll", "emoji": "🔄"}
        ]
...
async def test_task_flow():
    ...
    success = (
        final_task['status'] == 'completed' and
        final_task['progress'] == 100 and
        final_task.get('image_url') is not None and
        len(final_task.get('actions', [])) == 9  # Should have 9 action buttons
    )
    ...
    if success:
        ...
        if len(final_task.get('actions', [])) != 9:
            print(f"   • Should have 9 action buttons, got {len(final_task.get('actions', []))}")
