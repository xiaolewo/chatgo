                        ...
                        def find_video_fields(obj, path=""):
                            ...
                            if isinstance(obj, dict):
                                for key, value in obj.items():
                                    ...
                                    if isinstance(value, str) and any(ext in value.lower() for ext in ['.mp4', '.avi', '.mov', 'video', 'http']):
                                        video_fields.append((current_path, value))
                                    elif isinstance(value, (dict, list)):
                                        video_fields.extend(find_video_fields(value, current_path))
                            elif isinstance(obj, list):
                                for i, item in enumerate(obj):
                                    current_path = f"{path}[{i}]" if path else f"[{i}]"
                                    video_fields.extend(find_video_fields(item, current_path))
                            return video_fields
