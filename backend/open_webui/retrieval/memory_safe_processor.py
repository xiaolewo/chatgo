...
class SafeDocumentProcessor:
    ...
    def process_documents(
        ...
        try:
            ...
            log.info(f"Document processing completed. Processed: {processed_count}, Failed: {failed_count}")
...
class OptimizedQueryProcessor:
    ...
    def query_collections(
        ...
        actual_workers = min(
            self.max_workers,
            len(collection_names) * len(queries),
            10  # 最大10个线程
        )
        ...
        with ThreadPoolExecutor(max_workers=actual_workers) as executor:
            ...
            for query_idx, (query, query_embedding) in enumerate(zip(queries, query_embeddings)):
