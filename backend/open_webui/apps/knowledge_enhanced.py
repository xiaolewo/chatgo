...
from sqlalchemy import Column, String, BigInteger, Boolean, Text, Integer, JSON, ForeignKey
...
class KnowledgeVersion(Base):
    ...
    parent_version_id = Column(String, ForeignKey("knowledge_version.id"), nullable=True)
...
class KnowledgeStatistics(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    knowledge_id = Column(String, ForeignKey("knowledge.id"), nullable=False, unique=True)
...
class EnhancedPermissionManager:
    ...
    def check_knowledge_access(
        ...
        try:
            ...
            # 检查知识库级别的访问控制
            if hasattr(knowledge_obj, 'access_control') and knowledge_obj.access_control:
                from open_webui.utils.access_control import has_access
                
                action_str = action.value if isinstance(action, PermissionType) else action
    ...
    def _check_document_permissions(
        ...
        with get_db() as db:
            ...
            permissions = query.filter(
                ...
                (DocumentPermissions.group_id.in_(user_groups) if user_groups else False)
            ).all()
    ...
    def grant_permission(
        ...
        try:
            with get_db() as db:
                ...
                if is_group:
                    existing = existing.filter(DocumentPermissions.group_id == grantee_id)
                else:
                    existing = existing.filter(DocumentPermissions.user_id == grantee_id)
                
                if existing.first():
                    log.info(f"Permission already exists for {grantee_id} on {document_id}")
                    return True
                ...
                log.info(f"Granted {permission_type.value} permission to {grantee_id} on {document_id}")
                return True
    ...
    def revoke_permission(
        ...
        try:
            with get_db() as db:
                ...
                log.info(f"Revoked {deleted} permissions for {grantee_id} on {document_id}")
                return deleted > 0
    ...
    def get_logs(
        ...
        try:
            with get_db() as db:
                ...
                logs = query.order_by(KnowledgeAuditLog.timestamp.desc()).limit(limit).all()
    ...
    def create_version(
        ...
        try:
            with get_db() as db:
                ...
                # 获取版本号
                last_version = db.query(KnowledgeVersion).filter(
                    KnowledgeVersion.knowledge_id == knowledge_id
                ).order_by(KnowledgeVersion.version_number.desc()).first()
                ...
                log.info(f"Created version {version_number} for knowledge {knowledge_id}")
                return version.id
        ...
        try:
            with get_db() as db:
                versions = db.query(KnowledgeVersion).filter(
                    KnowledgeVersion.knowledge_id == knowledge_id
                ).order_by(KnowledgeVersion.version_number.desc()).limit(limit).all()
        ...
        try:
            with get_db() as db:
                ...
                if restore_version_id:
                    ...
                    log.info(f"Restored knowledge {knowledge_id} to version {version.version_number}")
                    return True
...
class StatisticsManager:
    ...
    def update_statistics(
        ...
        try:
            with get_db() as db:
                stats = db.query(KnowledgeStatistics).filter(
                    KnowledgeStatistics.knowledge_id == knowledge_id
                ).first()
                    ...
                    if details and 'query_time' in details:
                        ...
                        if stats.avg_query_time == 0:
                            ...
                            stats.avg_query_time = int(
                                (stats.avg_query_time * (stats.total_queries - 1) + details['query_time']) 
                                / stats.total_queries
                            )
                ...
                usage_by_day = {k: v for k, v in usage_by_day.items() if k >= cutoff_date}
    ...
    def get_analytics_report(
        ...
        if stats['usage_by_day']:
            ...
            report['trends'] = {
                ...
                'avg_daily_queries': total_queries / len(usage_data) if usage_data else 0,
