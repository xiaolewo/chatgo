...
from open_webui.models.kling_tasks import KlingTasks, KlingTaskForm, KlingTaskResponse, CameraControl
...
class KlingGenerateRequest(BaseModel):
    ...
    prompt: str = Field(..., min_length=1, max_length=2500, description="正向文本提示词")
    negative_prompt: Optional[str] = Field(None, max_length=2500, description="负向文本提示词")
...
@router.post("/config")
async def update_kling_config(request: Request, config: KlingConfig, user=Depends(get_admin_user)):
    """更新可灵配置"""
    try:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if "code" in result and "data" in result:
                        if result["code"] == 0:
                            ...
                            return {"message": f"API错误: {result.get('message', '未知错误')}", "status": "error"}
                    else:
                ...
                return {"message": f"{error_msg}: HTTP {response.status_code}", "status": "error"}
            elif response.status_code == 400:
                try:
                    error_detail = response.json()
                    error_msg = error_detail.get('message', '请求参数错误')
                    log.warning(f"API参数错误，但连接正常: {error_msg}")
                    return {"message": f"连接成功，但参数错误: {error_msg}", "status": "warning"}
                except:
...
async def generate_video(
    ...
    try:
        ...
        log.info(f"请求参数: prompt长度={len(generate_request.prompt)}, negative_prompt长度={len(generate_request.negative_prompt) if generate_request.negative_prompt else 0}")
        log.info(f"请求参数详情: model={generate_request.model_name}, mode={generate_request.mode}, aspect_ratio={generate_request.aspect_ratio}, duration={generate_request.duration}")
        ...
        if not kling_enabled:
            log.warning("可灵服务未启用")
            raise HTTPException(status_code=400, detail="可灵服务未启用，请先在管理员设置中启用并配置可灵服务")
        
        # 检查API配置
        api_url = getattr(request.app.state.config, "KLING_API_URL", "")
        api_key = getattr(request.app.state.config, "KLING_API_KEY", "")
        
        log.info(f"可灵API配置 - URL: {api_url[:50] if api_url else 'None'}..., Key: {'已配置' if api_key else 'None'}")
        
        if not api_url or not api_key:
            log.warning("可灵API配置不完整")
            raise HTTPException(status_code=400, detail="API配置不完整，请在管理员设置中配置可灵API URL和API Key")
        
        # 计算积分消耗并提前检查
        credits_cost = getattr(request.app.state.config, "KLING_PRO_CREDITS", 10) if generate_request.mode == "pro" else getattr(request.app.state.config, "KLING_STD_CREDITS", 5)
        ...
        try:
            ...
            if not user_credit or user_credit.credit < credits_cost:
                ...
                log.warning(f"用户 {user.id} 积分不足: 当前{current_credits}, 需要{credits_cost}")
                raise HTTPException(status_code=400, detail=f"积分不足，当前{current_credits}v豆，需要{credits_cost}v豆")
        ...
        if generate_request.camera_control:
            api_params["camera_control"] = generate_request.camera_control.dict(exclude_none=True)
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if api_result["code"] != 0:
                ...
                log.error(f"可灵API业务错误: code={api_result['code']}, message={error_msg}")
            ...
            try:
                ...
                Credits.update_credit_by_user_id(user.id, user_credit.credit - credits_cost)
                log.info(f"用户 {user.id} 积分扣除成功: {old_credits} -> {old_credits - credits_cost} (扣除{credits_cost})")
            except Exception as e:
            ...
            task_form_data = {
                ...
                "duration": generate_request.duration,
                "cfg_scale": generate_request.cfg_scale,
                "camera_control": generate_request.camera_control.dict() if generate_request.camera_control else None,
            ...
            log.info(f"视频生成任务成功提交到可灵平台: user_id={user.id}, task_id={task_id}, status={task_status}")
...
async def get_task_status(task_id: str, request: Request, user=Depends(get_verified_user)):
    """获取任务状态"""
    try:
        # 从数据库查找任务
        ...
        if stored_task.status in ["submitted", "processing"]:
            ...
            log.info(f"API配置 - URL: {api_url[:50] if api_url else 'None'}..., Key: {'已配置' if api_key else 'None'}")
                    ...
                    async with httpx.AsyncClient(timeout=30.0) as client:
                        ...
                        if response.is_success:
                            ...
                            if api_result.get("code") == 0 and "data" in api_result:
                                ...
                                new_status = task_data.get("task_status", stored_task.status)
                                ...
                                log.warning(f"外部API响应格式异常: code={api_result.get('code')}, data存在={('data' in api_result)}")
                        else:
                            log.error(f"外部API状态查询失败: HTTP {response.status_code} - {response_text}")
        ...
        log.info(f"返回任务状态: task_id={task_id}, status={stored_task.status}, video_url={stored_task.video_url}")
...
async def kling_callback(request: Request):
    ...
    try:
        ...
        log.info(f"收到可灵API回调通知: {json.dumps(callback_data, ensure_ascii=False, indent=2)}")
