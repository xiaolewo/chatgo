...
class ReferenceImage(BaseModel):
    ...
    weight: Optional[float] = Field(1.0, ge=0.1, le=3.0, description="图片权重 (0.1-3.0)")
    type: str = Field("reference", description="图片类型: reference(普通参考图) 或 style(风格参考图)")

class AdvancedParameters(BaseModel):
    """高级参数模型"""
    chaos: Optional[int] = Field(None, ge=0, le=100, description="混乱程度 (0-100)")
    stylize: Optional[int] = Field(None, ge=0, le=1000, description="风格化程度 (0-1000)")
    seed: Optional[int] = Field(None, ge=0, le=4294967295, description="种子值 (0-4294967295)")
    version: Optional[str] = Field(None, description="MidJourney版本 (v5.2, v6, v6.1, v7)")
    tile: Optional[bool] = Field(False, description="平铺模式")
    quality: Optional[float] = Field(1.0, ge=0.25, le=2.0, description="图像质量 (0.25-2.0)")
...
class ImageGenerateRequest(BaseModel):
    ...
    reference_images: Optional[List[ReferenceImage]] = Field([], description="参考图片列表")
...
async def get_midjourney_config(request: Request, user=Depends(get_admin_user)):
    ...
    return {
        ...
        "fast_credits": getattr(request.app.state.config, 'MIDJOURNEY_FAST_CREDITS', 10),
        "relax_credits": getattr(request.app.state.config, 'MIDJOURNEY_RELAX_CREDITS', 5),
        "turbo_credits": getattr(request.app.state.config, 'MIDJOURNEY_TURBO_CREDITS', 15)
    }
...
async def update_midjourney_config(
    ...
    request.app.state.config.MIDJOURNEY_TURBO_CREDITS = getattr(config, 'turbo_credits', 15)
...
async def generate_image(
    ...
    try:
        ...
        config_fast_credits = getattr(req.app.state.config, 'MIDJOURNEY_FAST_CREDITS', 10)
        config_relax_credits = getattr(req.app.state.config, 'MIDJOURNEY_RELAX_CREDITS', 5)
        config_turbo_credits = getattr(req.app.state.config, 'MIDJOURNEY_TURBO_CREDITS', 15)
        
        # 验证服务状态
        if not config_enabled:
            raise HTTPException(
                status_code=503, 
                detail="MidJourney服务未启用，请联系管理员开启服务"
            )
        ...
        if len(request.prompt) > 2000:
            raise HTTPException(
                status_code=400,
                detail="图像描述不能超过2000个字符"
            )
        
        if request.mode not in ["fast", "relax", "turbo"]:
            raise HTTPException(
                status_code=400,
                detail="生成模式必须是 'fast'、'relax' 或 'turbo'"
            )
        ...
        try:
            deduct_form = AddCreditForm(
                ...
                detail=SetCreditFormDetail(
                    ...
                    api_params={"mode": request.mode, "prompt": request.prompt[:50] + "..."},
        ...
        if request.reference_images:
            ...
            if reference_weights:
                avg_weight = sum(float(w) for w in reference_weights) / len(reference_weights)
                if avg_weight != 1.0:
        ...
        task_form = MidJourneyTaskForm(
            ...
            advanced_params=request.advanced_params.dict() if request.advanced_params else None,
            status="submitted",
            progress=0,
            message="任务已提交，等待处理",
        ...
        log.info(f"新的MidJourney任务已创建: {task_id}, 用户: {user.id}, 模式: {request.mode}")
        
        # 启动异步任务处理，传递当前配置
        asyncio.create_task(process_midjourney_task(task_id, config_api_url, config_api_key))
        ...
        try:
            refund_form = AddCreditForm(
                ...
                detail=SetCreditFormDetail(
                    ...
                    usage={"credits_refunded": task.credits_used, "reason": "task_cancelled"}
                )
            )
...
async def process_midjourney_task(task_id: str, config_api_url: str = None, config_api_key: str = None):
    """处理MidJourney任务的异步函数"""
    task = MidJourneyTasks.get_task_by_id(task_id)
    ...
    try:
        ...
        while poll_count < max_polls:
            ...
            log.info(f"任务进度更新: {task_id} - {task_info['progress']}% - {task_info['message']}")
            ...
            if mj_status == "SUCCESS":
                ...
                update_task_status(
                    ...
                    actions=actions,
                    seed=seed_value,
                    final_prompt=final_prompt if final_prompt else task_info.get("final_prompt")
                )
        ...
        try:
            refund_form = AddCreditForm(
                ...
                detail=SetCreditFormDetail(
                    ...
                    usage={"credits_refunded": task_info.get("credits_used", 0), "reason": "task_cancelled"}
                )
            )
            Credits.add_credit_by_user_id(refund_form)
            log.info(f"任务取消，已退还 {task_info.get('credits_used', 0)} v豆给用户 {task_info['user_id']}")
        except Exception as refund_error:
            log.error(f"退还v豆失败: {str(refund_error)}")
        ...
        try:
            refund_form = AddCreditForm(
                ...
                detail=SetCreditFormDetail(
                    ...
                    usage={"credits_refunded": task_info.get("credits_used", 0), "reason": "task_failed"}
                )
            )
            Credits.add_credit_by_user_id(refund_form)
            log.info(f"任务失败，已退还 {task_info.get('credits_used', 0)} v豆给用户 {task_info['user_id']}")
        except Exception as refund_error:
            log.error(f"退还v豆失败: {str(refund_error)}")
...
class ActionRequest(BaseModel):
    ...
    button_index: Optional[int] = Field(None, description="按钮索引 (0-3 for U1-U4/V1-V4)")
    custom_id: str = Field(..., description="按钮自定义ID")
...
async def execute_action(
    ...
    try:
        deduct_form = AddCreditForm(
            ...
            detail=SetCreditFormDetail(
                ...
                api_params={"action_type": request.action_type, "parent_task_id": task_id},
                usage={"credits_used": action_credits_needed, "action_type": request.action_type}
            )
        )
        Credits.add_credit_by_user_id(deduct_form)
        log.info(f"用户 {user.id} 消耗了 {action_credits_needed} v豆用于MidJourney动作操作")
    except Exception as e:
        log.error(f"扣除用户积分失败: {str(e)}")
        raise HTTPException(
            status_code=500,
    ...
    # 启动异步处理，传递当前配置
    asyncio.create_task(process_action_task(new_task_id, config_api_url, config_api_key))
    
    log.info(f"新的MidJourney动作任务已创建: {new_task_id}, 类型: {request.action_type}")
...
async def process_action_task(task_id: str, config_api_url: str = None, config_api_key: str = None):
    """处理MidJourney动作任务"""
    task = MidJourneyTasks.get_task_by_id(task_id)
    ...
    try:
        ...
        parent_task_model = MidJourneyTasks.get_task_by_id(parent_task_id) if parent_task_id else None
        parent_task = MidJourneyTasks.convert_to_legacy_format(parent_task_model) if parent_task_model else None
        ...
        if not action_response["success"]:
            raise Exception(f"动作API调用失败: {action_response.get('error', '未知错误')}")
        
        # 获取新的MidJourney任务ID
        new_mj_task_id = action_response["task_id"]
        
        update_action_task_status(
            mj_task_id=new_mj_task_id,
            message="动作已提交，正在处理",
            progress=20
        )
        ...
        try:
            refund_form = AddCreditForm(
                ...
                detail=SetCreditFormDetail(
                    ...
                    usage={"credits_refunded": task_info.get("credits_used", 0), "reason": "action_failed"}
                )
            )
            Credits.add_credit_by_user_id(refund_form)
            log.info(f"动作任务失败，已退还 {task_info.get('credits_used', 0)} v豆给用户 {task_info['user_id']}")
        except Exception as refund_error:
            log.error(f"退还v豆失败: {str(refund_error)}")
                ...
                for ref_img in reference_images:
                    ...
                    if isinstance(ref_img, dict):
                        ...
                        if base64_data:
                            ...
                            log.info(f"添加参考图片: {ref_img.get('filename', '未知文件名')}, 类型: {ref_img.get('type', 'unknown')}")
                    else:
                        log.warning(f"参考图片格式不正确: {type(ref_img)}")
            
            log.info(f"调用MidJourney API: {submit_url} (模式: {mode})")
            log.info(f"请求负载: prompt长度={len(payload['prompt'])}, 参考图数量={len(payload['base64Array'])}")
            if mode == "turbo":
...
async def call_midjourney_action_api(
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code != 200:
                raise Exception(f"动作API调用失败: {response.status_code} - {response.text}")
            
            result = response.json()
            if result.get("code") != 1:
                raise Exception(f"动作提交失败: {result.get('description', '未知错误')}")
...
async def fetch_midjourney_task(
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code != 200:
                raise Exception(f"任务查询失败: {response.status_code} - {response.text}")
