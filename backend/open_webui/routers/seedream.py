...
class LogoInfo(BaseModel):
    ...
    position: Optional[int] = Field(0, ge=0, le=3, description="水印位置：0-右下角,1-左下角,2-左上角,3-右上角")
    language: Optional[int] = Field(0, ge=0, le=1, description="水印语言：0-中文,1-英文")
...
class SeedreamGenerateRequest(BaseModel):
    """即梦3.0图像生成请求模型"""
    prompt: str = Field(..., min_length=3, max_length=2000, description="图像描述提示词")
    use_pre_llm: Optional[bool] = Field(False, description="开启文本扩写")
    seed: Optional[int] = Field(-1, description="随机种子，-1为随机")
    scale: Optional[float] = Field(2.5, ge=1.0, le=10.0, description="影响文本描述的程度")
...
async def get_seedream_config(request: Request, user=Depends(get_admin_user)):
    ...
    return {
        ...
        "credits_per_generation": getattr(request.app.state.config, 'SEEDREAM_CREDITS', 1)
    }
...
@router.post("/verify")
async def verify_seedream_connection(
    request: Request,
    user=Depends(get_admin_user)
):
    """验证即梦3.0连接"""
    try:
        ...
        if test_response["success"]:
            ...
            return {"message": f"连接验证失败: {test_response['error']}", "status": "failed"}
...
async def generate_image(
    req: Request,
    request: SeedreamGenerateRequest,
    ...
    try:
        ...
        if not config_enabled:
            raise HTTPException(
                status_code=503,
                detail="即梦3.0服务未启用，请联系管理员开启服务"
            )
        
        if not config_api_url or not config_api_key:
            raise HTTPException(
                status_code=503,
                detail="即梦3.0服务配置不完整，请联系管理员配置API信息"
            )
            ...
            try:
                refund_form = AddCreditForm(
                    ...
                    detail=SetCreditFormDetail(
                        ...
                        usage={"credits_refunded": config_credits, "reason": "task_creation_failed"}
                    )
                )
        ...
        try:
            ...
            if api_response["success"]:
                ...
                try:
                    refund_form = AddCreditForm(
                        ...
                        detail=SetCreditFormDetail(
                            ...
                            api_path="/seedream/generate",
                            api_params={"task_id": task_id},
                            usage={"credits_refunded": config_credits, "reason": "generation_failed"}
                        )
                    )
                    Credits.add_credit_by_user_id(refund_form)
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"图像生成失败: {api_response['error']}"
                )
            ...
            try:
                refund_form = AddCreditForm(
                    ...
                    detail=SetCreditFormDetail(
                        ...
                        api_path="/seedream/generate", 
                        api_params={"task_id": task_id},
                        usage={"credits_refunded": config_credits, "reason": "exception"}
                    )
                )
                Credits.add_credit_by_user_id(refund_form)
...
async def call_seedream_api(
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            log.info(f"请求负载: prompt长度={len(payload['prompt'])}, 尺寸={payload['width']}x{payload['height']}")
