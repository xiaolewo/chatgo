...
from open_webui.config import JIMENG_ENABLED, JIMENG_API_URL, JIMENG_API_KEY, JIMENG_CREDITS_5S, JIMENG_CREDITS_10S
...
@router.post("/config")
async def update_jimeng_config(request: Request, config: JimengConfig, user=Depends(get_admin_user)):
    """更新即梦配置"""
    try:
        # 更新配置值
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                return {"message": f"API Key无效或权限不足: HTTP {response.status_code}", "status": "error"}
            elif response.status_code == 400:
                log.warning(f"API参数错误，但连接正常")
                return {"message": "连接成功，但参数格式有误", "status": "warning"}
            else:
                return {"message": f"连接失败: HTTP {response.status_code}", "status": "error"}
...
async def generate_video(
    ...
    try:
        ...
        log.info(f"请求参数: prompt长度={len(generate_request.prompt)}, duration={generate_request.duration}, aspect_ratio={generate_request.aspect_ratio}")
        ...
        if not jimeng_enabled:
            log.warning("即梦服务未启用")
            raise HTTPException(status_code=400, detail="即梦服务未启用，请先在管理员设置中启用并配置即梦服务")
        
        # 检查API配置
        api_url = JIMENG_API_URL.value
        api_key = JIMENG_API_KEY.value
        
        log.info(f"即梦API配置 - URL: {api_url[:50] if api_url else 'None'}..., Key: {'已配置' if api_key else 'None'}")
        
        if not api_url or not api_key:
            log.warning("即梦API配置不完整")
            raise HTTPException(status_code=400, detail="API配置不完整，请在管理员设置中配置即梦API URL和API Key")
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            log.info(f"视频生成任务成功提交到即梦平台: user_id={user.id}, task_id={task_id}")
            
            # 启动后台任务轮询状态
            background_tasks.add_task(poll_task_status, request.app, task_id, api_url, api_key)
                ...
                if response.is_success:
                    ...
                    if result.get("code") == "success" and "data" in result:
                        ...
                        if stored_task:
                            ...
                            log.info(f"API响应原始数据: {json.dumps(result, ensure_ascii=False, indent=2)}")
                            log.info(f"task_data内容: {json.dumps(task_data, ensure_ascii=False, indent=2)}")
                            ...
                            if new_status == TaskStatus.FAILURE:
                                update_data["fail_reason"] = task_data.get("fail_reason", "未知错误")
                            ...
                            if new_status == TaskStatus.SUCCESS:
                                ...
                                # 尝试从video_url字段获取
                                    ...
                                    video_id = task_data.get("task_id")
                                    log.info(f"从路径 video_url 提取到视频URL: {video_url}")
                                ...
                                if video_url:
                                    ...
                                    log.info(f"成功提取视频信息 - URL: {video_url}, ID: {video_id}")
                                else:
                                    log.warning(f"任务 {task_id} 已成功但无法提取视频URL，task_data结构: {list(task_data.keys())}")
                            
                            # 更新到数据库
                            JimengTasks.update_task_by_id(task_id, update_data)
                            
                            log.info(f"任务 {task_id} 状态更新: {old_status} -> {new_status}")
                    ...
                    async with httpx.AsyncClient(timeout=30.0) as client:
                        ...
                        if response.is_success:
                            ...
                            if api_result.get("code") == "success" and "data" in api_result:
                                ...
                                # 更新数据库中的任务状态
                                log.info(f"手动查询API响应详细数据 - task_id: {task_id}")
                                log.info(f"API响应原始数据: {json.dumps(api_result, ensure_ascii=False, indent=2)}")
                                log.info(f"task_data内容: {json.dumps(task_data, ensure_ascii=False, indent=2)}")
                                ...
                                # 更新数据库
                                updated_task = JimengTasks.update_task_by_id(task_id, update_data)
                                if updated_task:
