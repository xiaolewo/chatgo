...
class PptTaskRequest(BaseModel):
    """PPT生成任务请求模型"""
    type: int = Field(..., description="生成类型：1-智能生成，2-文件生成，3-思维导图，4-Word转换，5-网页生成，6-文本生成，7-Markdown")
...
async def update_ppt_config(config: PptConfigModel, user=Depends(get_admin_user)):
    """更新PPT配置 (仅管理员)"""
    try:
        updated_config = PptConfigs.update_config(config.model_dump(exclude={"id", "created_at", "updated_at"}))
        if updated_config:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="获取模板选项失败")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="获取模板失败")
...
async def create_ppt_task(
    ...
    try:
        ...
        files_data = []
        for file in files:
            files_data.append(("file", (file.filename, await file.read(), file.content_type)))
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    log.info(f"PPT生成失败，为用户 {user.email} 退费 {credits_needed} v豆")
                    raise HTTPException(status_code=response.status_code, detail="创建任务失败")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="获取生成选项失败")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail=f"生成内容失败: {error_text}")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="修改内容失败")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail=f"生成PPT失败: {error_text}")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="获取PPT列表失败")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="加载PPT数据失败")
...
async def load_ppt_markdown(
    request: LoadPptMarkdownRequest,
    user=Depends(get_verified_user)
):
    """加载PPT大纲内容"""
    config = get_current_ppt_config()
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="加载PPT大纲失败")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="下载PPT失败")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=response.status_code, detail="下载动画PPT失败")
