...
from open_webui.models.agents import (
    AgentApps, AgentAppSubmissions, AgentAppFavorites, AgentAppStats,
    AgentApp, AgentAppSubmission, AgentAppFavorite
)
...
async def get_agent_app_by_id(
    ...
    try:
        ...
        # 检查访问权限
        if app.status != "active" and user.role != "admin" and app.created_by != user.id:
...
async def submit_agent_form(
    ...
    try:
        ...
        if not validation_result["is_valid"]:
            raise HTTPException(
                status_code=400, 
                detail={"message": "Form validation failed", "errors": validation_result["errors"]}
            )
...
def validate_form_data(form_config: Dict[str, Any], form_data: Dict[str, Any]) -> Dict[str, Any]:
    """验证表单数据"""
    errors = {}
    is_valid = True
...
async def process_uploaded_files(files: List[UploadFile], user_id: str) -> List[Dict[str, Any]]:
    """处理上传的文件"""
    file_info_list = []
...
async def process_ai_response(app: AgentApp, form_data: Dict[str, Any], files: List[Dict[str, Any]]) -> str:
    """处理AI响应（简化实现）"""
    try:
