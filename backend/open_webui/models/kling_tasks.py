...
class KlingTask(Base):
    ...
    # 任务状态
    status = Column(String, default="submitted")  # submitted, processing, succeed, failed
...
class KlingTaskForm(BaseModel):
    ...
    negative_prompt: Optional[str] = Field(None, max_length=200, description="负向文本提示词")
    ...
    def validate_model_name(cls, v):
        valid_models = ["kling-v1", "kling-v1-6", "kling-v2-master", "kling-v2-1-master"]
...
class KlingTasks:
    ...
    @staticmethod
    def insert_new_task(task_form_data: Dict[str, Any], user_id: str, task_id: str) -> Optional[KlingTask]:
        """创建新的可灵任务"""
        try:
    ...
    def get_task_by_id(task_id: str) -> Optional[KlingTask]:
        ...
        try:
            with SessionLocal() as session:
                task = session.query(KlingTask).filter(KlingTask.task_id == task_id).first()
                return task
        except Exception as e:
    ...
    @staticmethod
    def get_task_by_external_id(external_task_id: str, user_id: str) -> Optional[KlingTask]:
        """根据自定义任务ID获取任务"""
        try:
            with SessionLocal() as session:
                task = session.query(KlingTask).filter(
    ...
    def get_tasks_by_user_id(user_id: str, limit: int = 50) -> List[KlingTask]:
        ...
        try:
            with SessionLocal() as session:
                tasks = session.query(KlingTask).filter(
                    KlingTask.user_id == user_id
                ).order_by(
                    KlingTask.created_at.desc()
                ).limit(limit).all()
                return tasks
        except Exception as e:
    ...
    @staticmethod
    def update_task_by_id(task_id: str, update_data: Dict[str, Any]) -> Optional[KlingTask]:
        """更新任务信息"""
        try:
            with SessionLocal() as session:
                task = session.query(KlingTask).filter(
                    KlingTask.task_id == task_id
                ).first()
    ...
    def get_user_task_count(user_id: str) -> int:
        ...
        try:
            with SessionLocal() as session:
                count = session.query(KlingTask).filter(
                    KlingTask.user_id == user_id
                ).count()
                return count
        except Exception as e:
    ...
    def get_task_statistics() -> Dict[str, Any]:
        ...
        try:
            with SessionLocal() as session:
                ...
                succeed_tasks = session.query(KlingTask).filter(
                    KlingTask.status == "succeed"
                ).count()
                failed_tasks = session.query(KlingTask).filter(
                    KlingTask.status == "failed"
                ).count()
                processing_tasks = session.query(KlingTask).filter(
                    KlingTask.status.in_(["submitted", "processing"])
                ).count()
                ...
                return {
                    ...
                    "success_rate": (succeed_tasks / total_tasks * 100) if total_tasks > 0 else 0
                }
        except Exception as e:
