...
class JimengTask(Base):
    ...
    # 任务状态
    status = Column(String, default="SUBMITTED")  # NOT_START, SUBMITTED, QUEUED, IN_PROGRESS, SUCCESS, FAILURE
    ...
    # 时间戳
    created_at = Column(BigInteger, default=lambda: int(time.time() * 1000))  # 使用毫秒时间戳保持兼容性
...
class JimengTasks:
    ...
    def get_task_by_id(task_id: str) -> Optional[JimengTask]:
        ...
        try:
            with SessionLocal() as session:
                task = session.query(JimengTask).filter(
                    JimengTask.task_id == task_id
                ).first()
                return task
        except Exception as e:
    ...
    def get_tasks_by_user_id(user_id: str, limit: int = 50) -> List[JimengTask]:
        ...
        try:
            with SessionLocal() as session:
                tasks = session.query(JimengTask).filter(
                    JimengTask.user_id == user_id
                ).order_by(
                    JimengTask.created_at.desc()
                ).limit(limit).all()
                return tasks
        except Exception as e:
    ...
    def get_all_tasks(limit: int = 100) -> List[JimengTask]:
        ...
        try:
            with SessionLocal() as session:
                tasks = session.query(JimengTask).order_by(
                    JimengTask.created_at.desc()
                ).limit(limit).all()
                return tasks
        except Exception as e:
    ...
    @staticmethod
    def update_task_by_id(task_id: str, update_data: Dict[str, Any]) -> Optional[JimengTask]:
        """更新任务信息"""
        try:
            with SessionLocal() as session:
                task = session.query(JimengTask).filter(
                ...
                for key, value in update_data.items():
                    if hasattr(task, key):
                        # 特殊处理JSON数据
                        if key in ['request_params', 'api_response'] and isinstance(value, dict):
                ...
                # 如果任务完成或失败，设置完成时间
                if update_data.get('status') in ['SUCCESS', 'FAILURE'] and not task.completed_at:
    ...
    def get_user_task_count(user_id: str) -> int:
        ...
        try:
            with SessionLocal() as session:
                count = session.query(JimengTask).filter(
                    JimengTask.user_id == user_id
                ).count()
                return count
        except Exception as e:
    ...
    def get_task_statistics() -> Dict[str, Any]:
        ...
        try:
            with SessionLocal() as session:
                ...
                success_tasks = session.query(JimengTask).filter(
                    JimengTask.status == "SUCCESS"
                ).count()
                failed_tasks = session.query(JimengTask).filter(
                    JimengTask.status == "FAILURE"
                ).count()
                processing_tasks = session.query(JimengTask).filter(
