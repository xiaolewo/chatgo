...
class AgentApp(Base):
    ...
    category = Column(String, default="general")             # 分类：general, productivity, creative, analysis
...
class AgentAppSubmission(Base):
    ...
    status = Column(String, default="pending")             # 状态：pending, processing, completed, failed
        ...
        try:
            with get_db() as db:
                ...
                query = query.order_by(
                    AgentApp.usage_count.desc(),
                    AgentApp.created_at.desc()
                )
...
class AgentAppSubmissions:
    ...
    def get_submission_by_id(submission_id: str) -> Optional[AgentAppSubmission]:
        ...
        try:
            with get_db() as db:
                return db.query(AgentAppSubmission).filter(
                    AgentAppSubmission.id == submission_id
                ).first()
        except Exception as e:
    ...
    def update_submission_response(
        ...
        try:
            with get_db() as db:
                submission = db.query(AgentAppSubmission).filter(
                    AgentAppSubmission.id == submission_id
                ).first()
        ...
        try:
            with get_db() as db:
                ...
                return query.order_by(
                    AgentAppSubmission.created_at.desc()
...
class AgentAppFavorites:
    ...
    def add_favorite(app_id: str, user_id: str) -> bool:
        ...
        try:
            with get_db() as db:
                # 检查是否已收藏
                existing = db.query(AgentAppFavorite).filter(
    ...
    def remove_favorite(app_id: str, user_id: str) -> bool:
        ...
        try:
            with get_db() as db:
                favorite = db.query(AgentAppFavorite).filter(
                    AgentAppFavorite.app_id == app_id,
                    AgentAppFavorite.user_id == user_id
                ).first()
    ...
    def get_user_favorites(user_id: str) -> List[str]:
        ...
        try:
            with get_db() as db:
                favorites = db.query(AgentAppFavorite.app_id).filter(
                    AgentAppFavorite.user_id == user_id
                ).all()
