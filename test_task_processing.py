...
async def test_task_processing():
    ...
    print(f"📊 Initial status: {task_info['status']}, progress: {task_info['progress']}%")
    ...
    print(f"📊 Final status: {final_task['status']}, progress: {final_task['progress']}%")
    print(f"💬 Final message: {final_task['message']}")
    print(f"🖼️  Image URL: {'✅ Generated' if final_task.get('image_url') else '❌ Missing'}")
...
async def test_action_processing():
    ...
    print(f"🖼️  Action image URL: {'✅ Generated' if final_action_task.get('image_url') else '❌ Missing'}")
