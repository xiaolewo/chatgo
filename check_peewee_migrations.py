...
class PeeweeMigrationChecker:
    def __init__(self, migrations_dir: str):
        ...
        self.field_history: Dict[str, Dict[str, List[str]]] = {}  # table -> field -> [migration_files]
    ...
    def extract_add_fields_operations(self, file_path: Path) -> List[Tuple[str, List[str]]]:
        ...
        for migration_file in migration_files:
            ...
            for table_name, field_names in operations:
                ...
                for field_name in field_names:
                    ...
                    self.field_history[table_name][field_name].append(migration_file.name)
        ...
        for table_name, fields in self.field_history.items():
            for field_name, migration_files in fields.items():
                if len(migration_files) > 1:
                    ...
                    print_warning(f"表 '{table_name}' 的字段 '{field_name}' 在多个迁移中被添加:")
        ...
        for migration_file in migration_files:
            try:
                ...
                if 'add_fields' in content and 'try:' not in content:
                    print_warning(f"{migration_file.name}: 使用add_fields但没有错误处理")
                
                # 检查是否有已知的问题字段
                problematic_fields = ['share_id', 'timestamp', 'updated_at', 'created_at']
                for field in problematic_fields:
                    if f'{field}=' in content and 'duplicate column' not in content:
                        print_warning(f"{migration_file.name}: 添加了可能冲突的字段 '{field}'")
